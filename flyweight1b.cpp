...
class Student {
   private:
    shared_ptr<StudentShared> shared;
    string id;

   public:
    Student(shared_ptr<StudentShared> s, string id) {
        shared = s;
        this->id = id;
    }
...
class StudentFactory {
   private:
    ...
    map<string, shared_ptr<StudentShared>> studentMap;
   ...
   public:
    ...
    Student createStudent(const string& type, string id) {
        ...
        shared_ptr<StudentShared> studentS;
        ...
        if (studentMap.find(type) != studentMap.end()) {
            ...
            if (type == "Environment"){
                studentS = make_shared<StudentEnvironment>("Science", "Environment");
            }
            if (type == "Physics"){
                studentS = make_shared<StudentEnvironment>("Science", "Physics");
            }
            // 僅用來證明共同內容的確共用
            cout << studentS << endl;
...
int main() {
    ...
    auto factory = make_unique<StudentFactory>();
