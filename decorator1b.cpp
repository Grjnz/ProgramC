...
class BasicPlanWrapper : public PlanInterface {
   private:
    ...
    shared_ptr<PlanInterface> basicPlan;

   public:
    // 引數即為要被包裹的component instance或concrete decorators
    BasicPlanWrapper(shared_ptr<PlanInterface> bp) {
        basicPlan = bp;
    }
...
class SurplusVoice : public BasicPlanWrapper {
   ...
   public:
    ...
    SurplusVoice(shared_ptr<PlanInterface> bp) : BasicPlanWrapper(bp) {
        fee = 1000.0;
    };
    ~SurplusVoice() {
...
class SurplusInternet : public BasicPlanWrapper {
   ...
   public:
    SurplusInternet(shared_ptr<PlanInterface> bp) : BasicPlanWrapper(bp) {
        fee = 10000.0;
    }
    ~SurplusInternet() {
...
class SurplusMessage : public BasicPlanWrapper {
   ...
   public:
    SurplusMessage(shared_ptr<PlanInterface> bp) : BasicPlanWrapper(bp) {
        fee = 100000.0;
    }
    ~SurplusMessage() {
...
int main(void) {
    auto planX = make_unique<SurplusVoice>(
                 make_unique<BasicPlanA>());
    auto planXY = make_unique<SurplusInternet>(
                  make_unique<SurplusVoice>(
                  make_unique<BasicPlanA>()));
    auto planXYZ = make_unique<SurplusMessage>(
                   make_unique<SurplusInternet>(
                   make_unique<SurplusVoice>(
                   make_unique<BasicPlanA>())));
